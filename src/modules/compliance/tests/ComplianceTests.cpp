...
{
    protected:
        const char* m_expectedMmiInfo = "{\"Name\": \"Compliance\","
        ...
        const char* m_ComplianceModuleName = "OSConfig Compliance module";
        const char* m_ComplianceComponentName = "Compliance";
        ...
        const char* m_clientName = "ComplianceTest";
...
TEST_F(ComplianceTest, MmiOpen)
{
    MMI_HANDLE handle = nullptr;
    EXPECT_NE(nullptr, handle = ComplianceMmiOpen(m_clientName, m_normalMaxPayloadSizeBytes));
    ComplianceMmiClose(handle);
}
...
TEST_F(ComplianceTest, MmiGetInfo)
{
    char* payload = nullptr;
    char* payloadString = nullptr;
    int payloadSizeBytes = 0;

    EXPECT_EQ(MMI_OK, ComplianceMmiGetInfo(m_clientName, &payload, &payloadSizeBytes));
    EXPECT_NE(nullptr, payload);
    EXPECT_NE(0, payloadSizeBytes);
    ...
    ComplianceMmiFree(payload);
}

TEST_F(ComplianceTest, MmiSetInvalidComponent)
{
    ...
    EXPECT_NE(nullptr, handle = ComplianceMmiOpen(m_clientName, m_normalMaxPayloadSizeBytes));
    EXPECT_EQ(EINVAL, ComplianceMmiSet(handle, "TestComponentName", "remediateEnsurePermissionsOnEtcIssue", (MMI_JSON_STRING)payload, payloadSizeBytes));
    ComplianceMmiClose(handle);
}

TEST_F(ComplianceTest, MmiSetInvalidObject)
{
    MMI_HANDLE handle = NULL;
    const char* payload = "644";
    int payloadSizeBytes = strlen(payload);
    ...
    EXPECT_NE(nullptr, handle = ComplianceMmiOpen(m_clientName, m_normalMaxPayloadSizeBytes));
    EXPECT_EQ(EINVAL, ComplianceMmiSet(handle, m_ComplianceComponentName, "TestObjectName", (MMI_JSON_STRING)payload, payloadSizeBytes));
    ComplianceMmiClose(handle);
}

TEST_F(ComplianceTest, MmiSetOutsideSession)
{
    MMI_HANDLE handle = NULL;
    const char* payload = "644";
    int payloadSizeBytes = strlen(payload);

    EXPECT_EQ(EINVAL, ComplianceMmiSet(handle, m_ComplianceComponentName, "remediateEnsurePermissionsOnEtcIssue", (MMI_JSON_STRING)payload, payloadSizeBytes));
...
TEST_F(ComplianceTest, MmiGet)
{
    ...
    {
        EXPECT_EQ(MMI_OK, ComplianceMmiGet(handle, m_ComplianceComponentName, mimObjects[i], &payload, &payloadSizeBytes));
        EXPECT_NE(nullptr, payload);
        EXPECT_NE(0, payloadSizeBytes);
        EXPECT_NE(nullptr, payloadString = CopyPayloadToString(payload, payloadSizeBytes));
        EXPECT_EQ(strlen(payloadString), payloadSizeBytes);
...
TEST_F(ComplianceTest, MmiGetTruncatedPayload)
{
    MMI_HANDLE handle = NULL;
    char* payload = nullptr;
    char* payloadString = nullptr;
    ...
    EXPECT_NE(nullptr, handle = ComplianceMmiOpen(m_clientName, m_truncatedMaxPayloadSizeBytes));
    EXPECT_EQ(MMI_OK, ComplianceMmiGet(handle, m_ComplianceComponentName, m_auditEnsurePermissionsOnEtcIssueObject, &payload, &payloadSizeBytes));
    EXPECT_NE(nullptr, payload);
    EXPECT_NE(0, payloadSizeBytes);
    EXPECT_NE(nullptr, payloadString = CopyPayloadToString(payload, payloadSizeBytes));
    EXPECT_EQ(strlen(payloadString), payloadSizeBytes);
...
TEST_F(ComplianceTest, MmiGetInvalidComponent)
{
    MMI_HANDLE handle = NULL;
    char* payload = nullptr;
    int payloadSizeBytes = 0;
    ...
    EXPECT_EQ(EINVAL, ComplianceMmiGet(handle, "Test123", m_ComplianceComponentName, &payload, &payloadSizeBytes));
...
TEST_F(ComplianceTest, MmiGetInvalidObject)
{
    MMI_HANDLE handle = NULL;
    char* payload = nullptr;
    int payloadSizeBytes = 0;
    ...
    EXPECT_EQ(EINVAL, ComplianceMmiGet(handle, m_ComplianceComponentName, "Test123", &payload, &payloadSizeBytes));
    EXPECT_EQ(nullptr, payload);
    EXPECT_EQ(0, payloadSizeBytes);
...
TEST_F(ComplianceTest, MmiGetOutsideSession)
{
    MMI_HANDLE handle = NULL;
    char* payload = nullptr;
    int payloadSizeBytes = 0;

    EXPECT_EQ(EINVAL, ComplianceMmiGet(handle, m_ComplianceComponentName, m_auditEnsurePermissionsOnEtcIssueObject, &payload, &payloadSizeBytes));
    EXPECT_EQ(nullptr, payload);
    EXPECT_EQ(0, payloadSizeBytes);

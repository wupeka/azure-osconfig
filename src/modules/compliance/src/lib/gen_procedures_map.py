        ...
        with open(filepath, 'r') as file:
            for line in file:
                ...
                if audit_match:
                    ...
                    procedure_map[procedure_name][0] = "Audit_{}".format(procedure_name)
                ...
                if remediate_match:
                    ...
                    if procedure_name not in procedure_map:
                        procedure_map[procedure_name] = [None, None]
                    procedure_map[procedure_name][1] = "Remediate_{}".format(procedure_name)
...
""")
    ...
    for procedure_name, functions in procedure_map.items():
        if functions[0]:
            cpp_file.write('Result<bool> {}(std::map<std::string, std::string> args, std::ostringstream &logstream);\n'.format(functions[0]))
        if functions[1]:
            cpp_file.write('Result<bool> {}(std::map<std::string, std::string> args, std::ostringstream &logstream);\n'.format(functions[1]))
    cpp_file.write('std::map<std::string, std::pair<action_func_t, action_func_t> > Evaluator::mProcedureMap = {\n')
